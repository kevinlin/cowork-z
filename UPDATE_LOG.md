...
- **Fix: Content Security Policy was disabled (#1)** — `tauri.conf.json` had `"csp": null`, so any markup-injection bug anywhere in the renderer escalated to arbitrary script execution with access to all granted IPC commands. The app now ships a restrictive CSP (`default-src 'self'`, no inline scripts in production; `asset:` scheme allowed for media previews; `ipc:` for Tauri IPC; `data:` objects for the PDF preview) plus a dev-only variant that permits Vite HMR. Note: inline scripts inside agent-generated HTML previews no longer execute in production builds — static HTML still renders, and files can be opened externally for interactive use. (Technical review finding #1.)
- **Fix: `asset:` protocol granted the webview read access to the entire filesystem (#2)** — The asset protocol scope was `["**"]`, letting any code in the webview load any local file (SSH keys, browser profiles) — arbitrary local-file exfiltration if combined with an XSS. The static scope is now empty and extended at runtime to exactly the directories previews need: registered workspaces, granted permission folders, and app-managed directories (skill repo cache, skill install folders). The scope re-syncs whenever workspaces are added/switched or folder permissions are granted. (Technical review finding #2.)
- **Fix: Renderer-reachable file commands operated on arbitrary paths (#3)** — `read_file_content`, `read_binary_file`, and `trash_file` accepted any absolute path, and `write_text_file` could write attacker-controlled content anywhere — turning any webview XSS into an arbitrary file read/write/delete primitive. File reads and trash operations are now validated through a canonicalizing path guard (resolving `..` and symlinks) against registered workspaces and granted permission folders, and `write_text_file` is replaced by `export_text_file`, which opens the native save dialog Rust-side so the renderer never supplies a write path. (Technical review finding #3.)
- **Fix: Git personal access tokens were persisted in plaintext to skill-repo `.git/config` (#8)** — Syncing a private skill repo injected the PAT into the remote URL via `git remote set-url`, leaving the keychain-sourced token in cleartext on disk. Pulls now pass the token-injected URL transiently on the `git pull` invocation (like clone already did), and any credentials persisted by earlier versions are scrubbed from the remote URL on the next sync. (Technical review finding #8.)
- **Fix: Webview windows held unused shell permissions and a filesystem-wide opener grant (#15)** — Both capability files granted shell permissions (including unscoped `shell:allow-execute` on the skills window) that no frontend code uses — the sidecar is spawned Rust-side and skill-repo git operations run in Rust. These grants and the `opener:allow-open-path` scope of `**` were pure unused IPC attack surface. Shell grants are removed from both webview capability files, and the opener path scope is narrowed to `$HOME/**` plus external-volume mounts, matching where workspaces are allowed. (Technical review finding #15.)
- **Fix: HTML preview allowed sandboxed agent content to escape via popups (#5)** — Agent-written `.html` files were previewed in an iframe with `allow-popups-to-escape-sandbox`, letting any script `window.open()` a page that escapes the sandbox entirely; `baseHref` was also interpolated into `<base href="…">` without quote-escaping. The sandbox now grants only `allow-scripts allow-forms` (scripts run in an opaque origin with no Tauri API access), and `baseHref` is HTML-encoded before interpolation. Use "open externally" for full-fidelity rendering. (Technical review finding #5.)
- **Fix: MCP secrets, HTTP bodies, and SSE payloads were logged in plaintext (#10)** — MCP server configs (whose `environment`/`headers` maps typically hold API tokens), every HTTP response body, and every SSE event (full conversation content) were written to unrotated plaintext logs and forwarded over IPC. The sidecar log redaction now blanks all values inside `environment`/`headers` maps, and full HTTP/SSE payload logging is gated behind an explicit `SIDECAR_DEBUG_PAYLOADS=1` flag — by default only metadata (event type, method, path, status) is logged. (Technical review finding #10.)
- **Fix: OpenCode server password was written to plaintext logs and the debug panel (#4)** — The per-launch basic-auth password (which authenticates an API that can run shell commands) was logged verbatim to `~/.local/share/opencode/log/` and mirrored over IPC to the frontend debug panel. The log line now reports only the password length, and the sidecar logger gained a redaction pass that strips values of secret-looking keys (password, token, api-key, authorization, credential, …) and inline `KEY=value` assignments from every log entry before it reaches the file or IPC stream. (Technical review finding #4.)
- **Fix: API keys added or rotated mid-session were silently ignored (#14)** — API keys are applied as env vars only when the OpenCode server process spawns; once the sidecar was initialized, keys on subsequent `start_task`/`resume_session` payloads were dropped without warning, causing confusing provider errors until a full app restart. The sidecar now fingerprints incoming keys and, when they change, deliberately restarts the OpenCode server before the task starts (or logs a deferral warning if other sessions are still active). (Technical review finding #14.)
- **Fix: SSE reconnect could resurrect a disconnected stream and retried with no backoff (#13)** — The sidecar's SSE reconnect `setTimeout` handle was discarded, so a timer pending at `disconnect()` time still fired and re-enabled reconnection; against a permanently dead server the stream also retried every 5s forever. The timer is now stored and cleared on disconnect, the callback re-checks reconnect intent, and retries use exponential backoff capped at 60s (reset on successful connect). (Technical review finding #13.)
...
- P2 #1 — Azure legacy-key migration (replaces the read-time fallback):
- P2 #2 — Hashed API-key fingerprint:

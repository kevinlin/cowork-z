...
- **Fix: CSP hardened — no more `data:` plugin content, plus `base-uri`/`frame-ancestors`** — The Content Security Policy still allowed `<embed>`/`<object>` elements with arbitrary base64 `data:` payloads (a markup-injection foothold), and lacked `base-uri`/`frame-ancestors`. PDF previews now load through the scoped asset protocol instead of inline base64 data URLs, `object-src` dropped `data:`, and `base-uri 'self'`/`frame-ancestors 'none'` were added. The now-unused `read_binary_file` IPC command was removed from the invoke surface (2026-06-12 review #29).
- **Fix: task persistence commands now validate the task id** — `save_task_message`, `save_task_status`, `save_task_session`, `save_task_summary`, and `complete_task` wrote to whatever `task_id` the renderer supplied with no existence check, so a renderer bug or injected call could corrupt another task's history or trigger completion side effects (including automation run transitions); all five now reject ids that don't reference a real task (2026-06-12 review #14).
- **Fix: database reads and lock acquisition no longer crash the app** — Most database read paths (task lists, messages, provider settings, workspaces, permissions, arenas) used `.expect()` and would panic — taking down the whole app — on any SQLite error, and a panic while holding the database/scheduler mutex would poison it so every later access also panicked. Reads now propagate errors to the caller as `Result`s, and all remaining `.lock().unwrap()` sites recover from poisoned mutexes instead of cascading the crash (2026-06-12 review #17).
- **Fix: database migrations are now transactional** — A crash or error in the middle of a schema migration could previously leave the database half-migrated (some statements applied, version not bumped), breaking subsequent launches. Each migration step now runs in its own transaction, so a failure rolls back cleanly to the last fully-applied version (2026-06-12 review #16).
- **Perf: streaming responses no longer re-render the whole page or re-parse markdown per delta** — Each streaming delta used to re-render the entire Execution page (the page subscribed to the whole task store) and run a full remark/GFM markdown parse of the growing message. The page now uses granular store selectors, the message list owns the streaming subscription, and markdown parsing during live streaming is throttled to once per 150ms with the rendered tree memoized between parses (2026-06-12 review #12).
- **Perf: chat message list no longer rescans all messages per row** — The "last assistant message" lookup ran inside the per-message render loop (O(n²) per render); it is now computed once per render (2026-06-12 review #35). List virtualization for very long sessions remains deferred.
- **Fix: Arena no longer writes duplicate message rows to the database** — While an Arena was open, every task event was persisted twice (once by the global task store, once by the arena store), and re-delivered events appended duplicate rows in the arena UI. Persistence is now single-owner (the global task store handles all task events; the arena store only persists finalized streaming messages, which the global handler skips for arena tasks), and arena message appends dedupe by message ID (2026-06-12 review #11).
- **Fix: event listeners no longer leak when components unmount quickly** — Several `listen()` registrations (automation events, native menu events, update checks) resolved asynchronously; if the component unmounted before registration completed, the unsubscribe was lost and the listener leaked, firing stale callbacks for the lifetime of the window. All listeners now route through a shared race-safe helper that cancels late-resolving registrations (2026-06-12 review #26).
- **Fix: the file tree now refreshes for changes in nested folders** — The workspace filesystem watcher only observed the root directory, so files the agent wrote into subfolders never triggered a sidebar/artifacts refresh and the tree looked stale; the watch is now recursive, with churn from always-hidden directories (`.git`, `node_modules`) filtered out and one refresh event per debounce batch (2026-06-12 review #19).

...
- **Fix macOS release build failure (undici / Node 20 incompatibility)** — Release builds had been failing on both macOS jobs (arm64 and x64) since mid-June. The app compiled, signed, and notarized correctly, then the build crashed the instant it entered DMG bundling with `TypeError: webidl.util.markAsUncloneable is not a function` on Node v20 (Tauri swallows the bundler subprocess output, so the failure first looked like an `hdiutil`/`bundle_dmg.sh` error; `--verbose` revealed the real Node crash). Root cause: a pnpm override `@actions/http-client>undici: ">=6.24.0"` (added during the v0.8.1 dependency overhaul) was open-ended and resolved undici to `8.4.1`, which requires `node >=22.19.0` — but the publish workflow runs Node 20. The dependency chain is `@tauri-release/cli → @actions/github → @actions/http-client → undici`. Constrained the override to `^6.24.0` so undici stays on the Node-20-compatible 6.x line (now `6.27.0`) while preserving the security floor. Also hardened the macOS jobs while diagnosing: pinned the runner to `macos-15`, switched the macOS step conditionals to `startsWith(matrix.platform, 'macos')`, added a Spotlight-disable pre-build step to pre-empt the separate `hdiutil` "Resource busy" race now that DMG bundling actually runs, and added `--verbose` to surface bundler output in CI logs.

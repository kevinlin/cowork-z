...
16. **Backgrounded release-build burst firing — completion path moved to Rust:** In macOS release builds, WKWebView throttles background JS, deferring the frontend's `complete_task` invoke. This held `is_running` indefinitely — every cron tick queued a pending run, then all drained back-to-back on foreground. A secondary race (`check-then-set` on `is_running`) between the cron thread and queue drainer caused duplicate dispatches ~80 ms apart. Three fixes:
    1. **Rust-side completion.** Extracted `handle_task_completion_internal` as a shared sync helper. `SidecarManager::handle_sidecar_event` calls it on `task_complete`; `complete_task` Tauri command delegates to the same helper. Frontend is no longer on the critical path.
    2. **Idempotency via DB-level CAS.** `try_complete_run_if_running` performs `UPDATE … WHERE status='running'`; only the caller whose UPDATE affects a row proceeds to release `is_running` and drain pending runs.
    3. **`is_running` race closed with `compare_exchange`.** Both `fire_automation_on_thread` and `process_pending_runs` claim the dispatch slot via `compare_exchange(false, true, SeqCst, SeqCst)`. Losers queue as `pending` (cron) or return (drainer). `dispatch_automation_run` assumes caller won the CAS; releases slot on early-error paths.

17. **`AutomationRunsPanel` overflow pushed pinned panels off-screen:** The panel's root used `flex h-full flex-col`, but `h-full` does not constrain a flex child to its parent's remaining space — it makes the child grow with its content. With many runs, the tab grew taller than the viewport and pushed the pinned `FoldersPanel` and `Todos` sections (rendered after the tab content in `Sidebar.tsx`) below the fold. The fix matches the sizing pattern used by `SessionPanel`/`FileTreePanel`: the panel root is now `flex min-h-0 flex-1 flex-col overflow-hidden`, the filter toolbar gets `shrink-0` so it never collapses, and the runs list keeps `min-h-0 flex-1 overflow-y-auto` so only the list scrolls — leaving the pinned sidebar sections fixed regardless of run count.

18. **Dispatch slot RAII (§2 architecture review):** The single-run slot was a raw `AtomicBool is_running` acquired in two places and released in five. `run_automation_now` dispatched without acquiring at all, while its completion path unconditionally released. A manual Run Now finishing could therefore free the slot mid-scheduled-run and drain pending runs on top of it (two concurrent runs, invariant violated). The fix: a new `dispatch_slot` module owns the contract (`DispatchSlot::try_acquire()` → `SlotGuard`, release-on-drop); `AutomationSchedulerState` parks the guard keyed by `run_id`, and `release(run_id)` only drops a matching holder, so a foreign completion releases nothing. `run_automation_now` was folded into the shared `dispatch_automation_run` seam: it now acquires the slot (returning "An automation run is already in progress" when busy, surfaced as a frontend toast), sets `tasks.automation_run_id` (previously omitted by its copy-pasted dispatch path), and dispatches fire-and-forget via `spawn_start_task_dispatch`. The now-unused `dispatch_start_task` helper was removed. See [plan_dispatch-slot-raii-pure-con-module.md](plan_dispatch-slot-raii-pure-con-module.md).

19. **Pure `cron_schedule` module (§3 architecture review):** `normalize_cron`, the dow-remap helpers, and `compute_next_fire` were private methods on `AutomationSchedulerRegistry`, with `normalize_cron_public` existing only so `validate_cron` could reach them, and zero tests on any of it. They are now free functions `normalize()` / `next_fire()` in `src-tauri/src/cron_schedule.rs` with table-driven unit tests (5-field prepend, dow remap ranges/lists/steps, invalid-expression handling); the registry and `validate_cron` both call the module and the escape hatch is deleted. On the frontend, `AutomationCard`'s independent `cron[4]` parse (the third cron reader) was replaced with `detectFrequencyFromCron`, and `cron-utils.ts` gained its own Vitest table tests (`cron-utils.test.ts`) covering `buildCron` / `detectFrequencyFromCron` round-trips, `buildDisplay`, and `parseTimeTo24`, including the Unix Sunday=`'0'` convention. See [plan_dispatch-slot-raii-pure-con-module.md](plan_dispatch-slot-raii-pure-con-module.md).

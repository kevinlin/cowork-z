...
- **Post-implementation fixes (2026-02-21):**
  - **SkillsSidebar auto-refresh:** subscribe to `skills:changed` with debounced `refreshRoot()` so the file tree updates after install/update/delete — `SkillsSidebar.tsx`
  - **Hidden file filter toggle:** Eye/EyeOff button reusing `isHiddenEntry()` — `SkillsSidebar.tsx`, `FileTreePanel.tsx`
  - **Delete installed skill:** trash icon on installed-state cards → `skillsDeleteInstalled()` (backend already existed) — `SkillCard.tsx`, `RepoSkillsGrid.tsx`
  - **Human-readable cache dir names:** `repo_cache_dir()` now uses `derive_cache_dir_name(url)` (repo name with `/` → `_`) instead of a UUID — `git_ops.rs` (+3 tests), `db/skill_repos.rs`, `lib.rs` background sync, `SkillCard.tsx` View handler
  - **Skills Manager entry from `SkillsCatalog`:** footer link → `openSkillsManagerWindow()` — `SkillsCatalog.tsx`
  - **`openai/skills` repo adapter:** `discover_openai_skills()` enumerates dotfile-prefixed subdirs (`skills/.curated/`, `.system/`, `.experimental/` — listed in `OPENAI_SKILL_SUBDIRS`) since the convention scanner skips dotfile dirs; URL-matched on `github.com/openai/skills`, falls back to convention scan — `skill_discovery.rs`

- **Handle existing skills repo (2026-02-23):** Pre-clone cleanup in `skill_repos_add` — removes any stale DB entry + auth token for the same URL and deletes the on-disk cache dir before re-cloning. Added `get_skill_repo_by_url()` helper — `commands/skill_repos.rs`, `db/skill_repos.rs`.

- **GitLab PAT support + Remove Repo UI (2026-03-29):**
  - **GitLab PAT URLs:** `inject_token()` detects `glpat-` prefix and uses `oauth2:{token}@` format (vs. `{token}@` for GitHub/generic) — `git_ops.rs`
  - **Remove Repo button:** per-repo `Trash2` button with `window.confirm()` guard, wires the existing `skill_repos_remove` command via a new `removeRepo` action — `RepoToolbar.tsx`, `skillsManagerStore.ts`

- **SkillsSidebar TreeRow reuse + symlink installation (2026-04-25):**
  - **TreeRow reuse:** replaced the custom `FileTreeNodes` with the exported `TreeRow` from `FileTreePanel.tsx`, inheriting file icons, symlink badges, hover actions (Open in Finder, Move to trash), drag-and-drop, and selection state — `SkillsSidebar.tsx`
  - **Symlink install (Unix):** `skills_install_from_repo` now uses `std::os::unix::fs::symlink` to point the target skills folder at the repo cache; existing entries (real dirs or stale symlinks) are cleared first. Windows still copies + writes `.coworkz-checksum`/`.coworkz-source`. The symlink itself carries provenance on Unix — `commands/skill_repos.rs`
  - **Install detection:** switched from `.coworkz-checksum` existence to `skill_dir.exists()` so it works for both symlinks and copies — `commands/skill_repos.rs`
  - **Symlink-safe deletion:** `skills_delete_installed` branches on `symlink_metadata()` (`fs::remove_file` for symlinks, `fs::remove_dir_all` for real dirs) to avoid wiping the shared repo cache — `commands/skill_repos.rs`

- **`skills.sh` deep link on `SkillCard` (2026-05-07):**
  - **URL:** `https://skills.sh/<org>/<repo>/<skill-id>` from `RepoSkill.repoName` (lowercased) + `RepoSkill.skillId`. `buildSkillsShUrl()` returns `null` for malformed `repoName` so the link is hidden rather than broken — `SkillCard.tsx`
...
- **Frontend skill category derivation (2026-05-07):**
  ...
  - **Consumers:** `SkillCard.tsx` memoizes the derived category for the badge; `RepoSkillsGrid.tsx` memoizes a `Map<repoId:skillPath, string>` so filter tabs, search match, and the badge stay in lockstep.
  - No backend / migration changes; `repo_skills.category` is unchanged — this is a presentation-layer concern only.

- **`Development` skill category (2026-05-10):**
  - **Motivation:** software-engineering skills (e.g. `code-review-expert`, `systematic-debugging`, `test-driven-development`, `spec-lint`, `spec-coding`) collapsed to `General` because no taxonomy key matched their names, making them visually indistinguishable from miscellaneous skills in the Skills Manager grid and Home catalog.
  - **Change:** added `Development` to `SkillCategory`, `CATEGORY_COLORS` (green palette — `bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400`), and `CATEGORY_KEYWORDS`. Keyword set sourced from `~/.claude/skills/spec-lint/references/convention.md` (`spec`, `spec-driven`, `requirements`, `architecture`, `adr`, `changelog`) plus broader software-development vocabulary (`code review`, `codebase`, ` code `, `coding`, `programming`, `developer`, `engineering`, `devops`, ` test `, `testing`, ` tdd `, ` review `, `debug`, `refactor`, `lint`, ` commit `, ` branch `, ` merge `, `pull request`, ` pr `, ` git `, ` api `, ` ci `, ` cd `, `compile`, `deploy`, `bugfix`). Short / ambiguous tokens are space-padded for whole-word matching against the space-wrapped haystack.
  - **Ordering:** inserted between `Design` and `Document` so existing matches stay stable — "design review" still lands in `Design` (its `design` substring wins first) while "test report" maps to `Development` (the `' test '` keyword fires before `report` reaches `Document`).
  - **Scope:** presentation-layer only — `lib/skill-categories.ts`. The taxonomy now covers 12 non-`General` keys (was 11). No backend / migration / curated-repo changes; `getCuratedRepoCategories()` continues to return `[]` for user-added repos so the new key is opt-in via name keyword.

- **Refactor: shared `fs_utils` module (2026-05-08):**
  - **Motivation:** after the Unix symlink-install change, `compute_dir_checksum` / `collect_files` / `copy_dir_recursive` in `commands/skills.rs` were only used on the Windows fallback path (and in tests), producing 4 unused-code warnings on macOS. `commands/packs.rs` also held a duplicate `copy_dir_recursive`.
  - **New module:** `src-tauri/src/fs_utils.rs` (registered in `lib.rs` next to `git_ops`/`fs_watcher`) holds all three helpers + 6 unit tests. `copy_dir_recursive` keeps the safer `is_file()` branch from the `packs.rs` version (skips symlinks/sockets/fifos).
  - **`cfg` gating:** `#[cfg(any(not(unix), test))]` on `compute_dir_checksum` / `collect_files` (and the `sha2` import — still needed by tests on Unix); `#[cfg(not(unix))]` on `copy_dir_recursive`. Same gating mirrored on the consumer imports.
  - **Call sites:** `commands/packs.rs` → `crate::fs_utils::copy_dir_recursive`; `commands/skill_repos.rs` → `crate::fs_utils::{compute_dir_checksum, copy_dir_recursive}`. `commands/skills.rs` is now scoped purely to skill metadata listing/lookup.
...
- **Handle existing skills repo (2026-02-23):** Added pre-clone cleanup to `skill_repos_add` — if a repo with the same URL already exists in the DB (leftover from a previous add/remove), the old entry and its auth token are removed first. If the cache directory already exists on disk, it is deleted before cloning. Added `get_skill_repo_by_url()` helper to `db/skill_repos.rs`.
- **GitLab PAT support + Remove Repo UI (2026-03-29):**
  - **GitLab PAT URL construction:** Updated `inject_token()` in `git_ops.rs` to detect `glpat-` prefixed tokens and use `oauth2:{token}@` format as required by GitLab HTTP auth. GitHub and generic tokens continue using the existing `{token}@` format.
  - **Remove Repo button:** Added `removeRepo` action to `skillsManagerStore.ts` and a `Trash2` delete button to `RepoToolbar.tsx` (visible when a specific repo is selected). Uses `window.confirm()` for safety. Wires through to the existing `skill_repos_remove` backend command.
- **SkillsSidebar TreeRow reuse + symlink installation (2026-04-25):**
  - **TreeRow reuse:** Replaced the custom `FileTreeNodes` component in `SkillsSidebar.tsx` with the exported `TreeRow` from `FileTreePanel.tsx`, gaining file type icons, symlink overlay badges, hover action buttons (Open in Finder/default app, Move to trash), drag-and-drop, and selected state highlighting.
  - **Symlink installation (macOS/Linux):** Changed `skills_install_from_repo` to create a symbolic link (`std::os::unix::fs::symlink`) from the target skills folder to the repo cache source instead of copying. Existing entries (real dirs or stale symlinks) are removed before linking. Windows retains copy-based installs. Skips `.coworkz-checksum` / `.coworkz-source` on Unix since the symlink itself tracks provenance.
  - **Install detection:** Changed from `.coworkz-checksum` existence check to `skill_dir.exists()` for consistent detection across symlinked and copied installs.
  - **Symlink-safe deletion:** Updated `skills_delete_installed` to check `symlink_metadata()` before removal — symlinks are removed with `fs::remove_file`, real directories with `fs::remove_dir_all`, preventing accidental deletion of the shared repo cache.
- **`skills.sh` deep link on `SkillCard` (2026-05-07):**
  - **Goal:** Give every Skills Manager card a one-click jump to its public listing on [skills.sh](https://skills.sh/), so users can read the agent skills directory entry (audit badges, install count, etc.) without leaving the app context.
  - **URL convention:** `https://skills.sh/<org>/<repo>/<skill-id>`, where `<org>` and `<repo>` are derived from `RepoSkill.repoName` (`org/repo`) and **always lowercased**, and `<skill-id>` is `RepoSkill.skillId` (the kebab-case skill folder name produced by `skill_discovery.rs`). Examples: `https://skills.sh/anthropics/skills/skill-creator`, `https://skills.sh/jimliu/baoyu-skills/baoyu-youtube-transcript`.
  - **`SkillCard.tsx`:** Added `buildSkillsShUrl(repoName, skillId)` helper that splits `repoName` on `/`, lowercases org and repo, and returns `null` when any segment is missing (so the link is hidden for malformed entries instead of producing a broken URL). A new `skills.sh` text link sits next to the existing `View` link in the bottom action row, using the same `text-primary text-xs underline hover:no-underline` styling for visual consistency. Click handler calls `openExternal()` from `@/lib/tauri-api` (which delegates to `@tauri-apps/plugin-opener`) to open the URL in the user's default browser. Wrapped in a Radix `Tooltip` (400 ms delay) showing the full URL on hover, matching the truncated-text tooltip pattern already used elsewhere on the card.
  - **No backend / type changes:** `RepoSkill.repoName` and `RepoSkill.skillId` already carry the data needed; this is a pure presentation-layer addition.

- **Frontend skill category derivation (2026-05-07):**
  - **Problem:** Many third-party repos don't follow the `{category}/skills/{name}` convention, so the backend's `derive_category()` produced unhelpful values (`General`, or non-taxonomy directory names like `Curated`/`System` from the `openai/skills` adapter). The badge on `SkillCard` and the filter tabs in `RepoSkillsGrid` therefore looked uninformative.
  - **Helpers:** Added `deriveCategoryFromName(name)` and `deriveSkillCategory(skillName, repoCategories, fallback)` to `src/lib/skill-categories.ts`. Keyword map (`CATEGORY_KEYWORDS`) covers all 11 non-`General` keys from `CATEGORY_COLORS`; matches are case-insensitive substring against a space-padded haystack so short tokens (`' pm '`, `' ads'`) behave as whole-word. Added `getCuratedRepoCategories(repoName)` to `curatedSkillRepos.ts` for the second-step lookup; returns `[]` for user-added repos so they fall through to the backend value.
  - **Precedence:** name keyword match → first curated repo category → backend `skill.category` → `'General'`.
  - **`SkillCard.tsx`:** Replaced `getCategoryColorClass(skill.category)` with a `useMemo` over `deriveSkillCategory(skill.name, getCuratedRepoCategories(skill.repoName), skill.category)`; the badge text and color now reflect the derived value.
  - **`RepoSkillsGrid.tsx`:** Computes the derived category once per skill (memoized `Map<repoId:skillPath, string>`) so the filter tabs, search match, and `SkillCard` badge all agree. Without this, the badge could show a category that isn't selectable in the tab strip.

- **Refactor: shared `fs_utils` module for filesystem helpers (2026-05-08):**
  - **Problem:** After the Unix symlink-install change, `compute_dir_checksum`, `collect_files`, and `copy_dir_recursive` in `commands/skills.rs` only ran on Windows (or in tests), producing four `unused`/`dead_code` warnings on macOS builds. `commands/packs.rs` also had its own near-identical `copy_dir_recursive`, so the helper was duplicated across two modules.
  - **Step 1 — silence the warnings without deleting needed code:** Gated each helper with the matching `cfg`: `#[cfg(any(not(unix), test))]` on `compute_dir_checksum`/`collect_files` and the `sha2` import (still needed by tests on all platforms), `#[cfg(not(unix))]` on `copy_dir_recursive`. Mirrored the gating on the imports in `commands/skill_repos.rs`. `cargo check` on macOS is now warning-free; tests on macOS still cover the Windows-only checksum logic.
  - **Step 2 — extract a shared module:** Created `src-tauri/src/fs_utils.rs` (registered alongside `git_ops`/`fs_watcher` in `lib.rs`) and moved `copy_dir_recursive` there using the safer of the two existing implementations (explicit `is_file()` check skips symlinks/sockets/fifos; descriptive error messages). Replaced the call site in `commands/packs.rs` with `crate::fs_utils::copy_dir_recursive(...)` and dropped the local copy. Added 3 unit tests (`copies_nested_files_and_directories`, `errors_when_source_missing`, `creates_destination_directory`).
  - **Step 3 — co-locate the checksum helpers:** Moved `compute_dir_checksum`, `collect_files`, the gated `sha2`/`PathBuf` imports, and the 3 checksum tests (`test_compute_dir_checksum_stable`, `test_compute_dir_checksum_changes_on_edit`, `test_checksum_ignores_coworkz_checksum_file`) from `commands/skills.rs` into `fs_utils.rs`. `commands/skill_repos.rs` now imports both Windows-only helpers from one place: `#[cfg(not(unix))] use crate::fs_utils::{compute_dir_checksum, copy_dir_recursive};`. `commands/skills.rs` is now focused purely on skill metadata listing/lookup.
  - **Verification:** `cargo check` reports zero warnings; `cargo test --lib` passes 32/32 (including the 3 new copy tests and the 3 relocated checksum tests, now under `fs_utils::tests::`).
  - **No backend / migration changes:** the `repo_skills.category` column remains the path/manifest-derived value; the new logic is a pure presentation-layer concern.